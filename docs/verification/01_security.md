...
> 本文是当时那一批（安全模块）的真实验证快照，结论按当时状态记录；其后落地的能力（任务落盘与僵死回收、LLM 超时与重试、任务级隔离、产物校验与修复重试、文档解析工具、并发队列）不在本文范围内，见 README 与 git 历史。
...
第一批状态：**代码和离线验证完成**；真实沙箱配额与会话检查由 `python -m demo.security_smoke` 在具备 Docker 的环境执行。
