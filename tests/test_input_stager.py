...
def test_stages_object_under_workspace():
    ...
    record = stager.stage("task1", "raw/sales.xlsx")
    ...
    assert sandbox.writes == [
        {"filename": "tasks/task1/sales.xlsx", "content": b"xlsx-bytes"}
    ]
    assert sandbox.directories == ["/home/gem/workspace/tasks/task1"]


def test_explicit_filename_wins_and_nested_keys_use_the_basename():
    stager, _, sandbox = _stager({"raw/2026/q3/data.csv": b"a,b\n1,2\n"})
    record = stager.stage("task1", "raw/2026/q3/data.csv", "custom.csv")
    assert record["sandbox_path"] == "/home/gem/workspace/tasks/task1/custom.csv"
    assert sandbox.writes[0]["filename"] == "tasks/task1/custom.csv"
...
def test_unsafe_destination_names_are_rejected(filename):
    ...
    with pytest.raises(PermissionDenied):
        stager.stage("task1", "raw/x.csv", filename)
    assert storage.downloads == [] and sandbox.writes == []


def test_nested_but_safe_names_stay_inside_the_workspace():
    stager, _, sandbox = _stager({"raw/x.csv": b"x"})
    record = stager.stage("task1", "raw/x.csv", "incoming/x.csv")
    assert record["sandbox_path"] == "/home/gem/workspace/tasks/task1/incoming/x.csv"
    assert sandbox.writes[0]["filename"] == "tasks/task1/incoming/x.csv"
...
def test_sandbox_write_failure_becomes_a_staging_error():
    ...
    with pytest.raises(InputStagingError, match="cannot stage"):
        stager.stage("task1", "raw/x.csv", "incoming/x.csv")
...
def test_missing_object_fails_with_a_clear_error():
    ...
    with pytest.raises(InputStagingError, match="cannot load input"):
        stager.stage("task1", "raw/missing.csv")
    assert sandbox.writes == []
...
def test_empty_object_is_rejected():
    ...
    with pytest.raises(InputStagingError, match="empty"):
        stager.stage("task1", "raw/empty.csv")
    assert sandbox.writes == []
...
def test_stage_all_preserves_declared_fields_and_order():
    ...
    assert [item["sandbox_path"] for item in staged] == [
        "/home/gem/workspace/tasks/task1/a.csv",
        "/home/gem/workspace/tasks/task1/renamed.csv",
    ]
    assert [item["bytes"] for item in staged] == [3, 4]
    assert len(sandbox.writes) == 2


@pytest.mark.parametrize("task_id", ["../escape", "a/b", "", "x" * 65])
def test_unsafe_task_ids_are_rejected(task_id):
    stager, _, sandbox = _stager({"raw/x.csv": b"x"})
    with pytest.raises(PermissionDenied):
        stager.stage(task_id, "raw/x.csv")
    assert sandbox.writes == []
